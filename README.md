...
- 版を指定する: `curl -fsSL ... -o install.sh && sh install.sh 0.40.0`
...
```
help            使い方を表示する (haj help <名前> で個別、--quick で圧縮リファレンス)
...
config          設定の実効値と出所を見る (--init で雛形、get/set で読み書き)
...
tree            共有ツリーの取得と更新 (install/update/list/remove/configure)
```
...
ツリーが `docs/quickref.md` を持っていれば、`haj help --quick` がコアと全ツリー分を
連結して一枚の圧縮リファレンスにする(エディタや AI セッションに「いま何ができるか」を
渡す出口)。作り方は `haj docs trees`。

## プロジェクトのタスク

install / build / test のような「このリポジトリの作業」はタスクにする。コマンド(語彙)と
違って探索に乗らず、**現在のプロジェクトの `.haj/` だけ**を見る — 遡らない・上書き
しない・フォールバックしない。だから `haj run test` という1行は、どの環境で読んでも
「そのリポジトリのタスク」以外に解釈できない。
...
$ haj run test     # 実行(以降の引数はそのまま渡る)
```

1行で書けなくなったら `.haj/tasks/<名前>` の実行ファイルに昇格する。
...
口は2つ。どちらも値の実体ではなく**参照**を書き、平文をディスクに残さない。
参照の書式は `vault://` / `op://` / `env://` / `file://` など(`haj docs secrets`)。

**その実行だけ渡す(注入)** — フラグをサブコマンド名の前に書く:

```sh
haj --secret DB_PASS=vault://secret/data/db/password mig up
haj --secret DB_PASS=vault://secret/data/db/password secret check   # 何が渡るかを金庫に触らずに確認
```

**繰り返し使う秘密は宣言して、要る瞬間に引く(pull)** — 何を読めるかは宣言表が
決める。exec 時に何かが勝手に注入されることはない:

```
# ~/.config/haj/config
user.secret.OCI_KEY = vault://users/you/oci/private_key
```

```sh
OCI_CLI_KEY_FILE=$(haj secret file OCI_KEY) oci ...   # 0600 の一時ファイルに実体化してパスを返す
haj secret list                                       # 宣言の目録
haj secret check                                      # 宣言と受け渡しの検証(金庫に触らない)
```

ツリーのコマンドの中では宣言域が `tree.<インストール名>.secret.KEY` に切り替わり、
ツリー自身が書き込みたい秘密(トークン等)は `haj store put` で自ツリー専用の
名前空間に置く。
...
haj help --quick   # コアと全ツリーの圧縮リファレンスを一枚で出す
```
