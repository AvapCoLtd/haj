...
| `HAJ_START_DIR` | haj が起動された時点の cwd(`-C` 適用前)。「ユーザーが元居た場所」 |
| `HAJ_USER_CONFIG` | ユーザー設定ファイルのパス(XDG 解決済み。存在チェックは読み手がする) |
| `HAJ_TREE` | インストール済みツリー由来のときだけ: **インストール名**。ローカル状態の置き場をインスタンスごとに分けるのに使う(それ以外では未設定) |

これらは**コアが注入する**実行時情報。§3 の `--haj-env` で**申告する**のは
「コマンド自身が読む設定値」で、向きが逆(注入 vs 申告)。`HAJ_*` をコマンドの
設定値の名前に使わないこと。
