...
**宣言域は文脈で決まる**(相補): ツリーのコマンドの中(HAJ_TREE)は
`tree.<名前>.secret.*`、外(人間のシェル・個人/共通コマンド)は `user.secret.*`。
ツリーから user.* には届かない(clone したツリーに個人の宣言を引かせない)。
...
# ~/.config/haj/config: user.secret.OCI_KEY = vault://secret/data/oci/private_key
OCI_CLI_KEY_FILE=$(haj secret file OCI_KEY) oci compute instance list ...
```

`haj secret file <KEY>` は get のファイル版 — 値を
`$XDG_RUNTIME_DIR/haj/secret-files/<KEY>`(0600)に書いてパスを出す。同じ KEY は
