#!/bin/sh
# haj を GitHub Releases から入れる(公開版。トークン不要)。
#
#   curl -fsSL https://raw.githubusercontent.com/AvapCoLtd/haj/master/install-public.sh | sh
#   ./install-public.sh 0.11.0     版を指定して入れる
#
# avap 社内(Package Registry から入れる)は install.sh のほう。
set -eu

REPO="${HAJ_REPO:-AvapCoLtd/haj}"
PREFIX="${HAJ_PREFIX:-/usr/local}"

die() { echo "install-public.sh: $*" >&2; exit 1; }
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "リリースが見つかりません。版を明示してください: ./install-public.sh 0.11.0"
fi
