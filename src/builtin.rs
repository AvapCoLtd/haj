...
pub const ALL: &[Builtin] = &[
    ...
    Builtin {
        name: "store",
        describe: "自ツリーのストアを読み書きする (get / put / login / status)",
    },
    Builtin {
        name: "exec",
...
タスクとツリーの合成形もある (SPEC §9.6 / §9.7):

  haj env run <タスク>            そのタスクの環境変数
  haj env <ツリー名> <名前>        そのツリーのコマンドの環境変数
  haj env run / haj env <ツリー名> 名前を省くと、応答する全コマンドの
                                  --haj-env を # ==== <名前> ==== の節で連結
...
足しても補完は自動で追従する (プロジェクト固有のコマンドもそのまま出る)。",
        ...
        "secrets" => "\
haj secrets は haj secret check に改名されました (SPEC §10.6)。

  haj secret get <KEY>    宣言 tree.<HAJ_TREE>.secret.<KEY> を解決して stdout へ
  haj secret list         宣言の一覧
  haj secret check        宣言と受け渡しの検証 (旧 haj secrets --check)"
            .to_string(),
...
pub fn complete(name: &str, words: &[String]) -> Vec<String> {
    match name {
        ...
        // 動詞と、get には宣言済みの KEY(手元の設定だけで列挙 — 金庫に触らない)
        "secret" => crate::secret::complete(words),
